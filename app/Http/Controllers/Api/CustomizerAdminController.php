...
class CustomizerAdminController extends Controller
{
    ...
    public function reorderCategoryOptions(Request $request, int $id): JsonResponse
    {
        ...
        $ownIds = $category->options()->pluck('id')->all();
        if (count($data['order']) !== count($ownIds) || array_diff($data['order'], $ownIds)) {
            return response()->json(['message' => "Order must be a permutation of this category's styles."], 422);
        }

        DB::transaction(function () use ($data) {
            foreach ($data['order'] as $position => $optionId) {
                LayerOption::where('id', $optionId)->update(['display_order' => $position]);
            }
        });
