...
**Not verified:** that Flitt accepts `lang` on a live token request. It is in the gateway's documented order parameters and the signature covers it, but minting a real token to see the Georgian page come back was not something this session could do — the next payment on kereforyou.com is what confirms it.
