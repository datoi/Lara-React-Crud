...
- **The checkout page came up in English.** Flitt accepts a `lang` parameter and lists `ka` among its languages, but `createCheckoutToken` never sent one — so the gateway chose for itself and a Georgian-default storefront handed its customers an English card form, seen on screen during this round of testing. The client now sends the locale it is displaying with `POST /api/orders/{id}/pay`, and `FlittService::languageFor()` decides whether that is a language this site speaks: `ka` and `en` pass, anything else falls back to `ka`. It falls back rather than refusing because the language is cosmetic and the payment is not — a junk value should cost the customer a Georgian page, not the order they are trying to pay for. The value is signed with the rest of the request, so it was never going to be passed through as it arrived.
...
- **The card marks now stand where the customer decides, not only in the footer.** `PaymentMarks` is a small shared component — the row is all it owns, the label and surrounding type come from the caller, because the footer and the cart summary sit on different backgrounds. The footer renders byte-for-byte what it did before; the cart summary gains the same row under a hairline, beneath "place order". It shares `footer.weAccept` deliberately rather than taking a `cart.*` copy of the same sentence, since two copies of "We accept:" are two things that can drift. **The checkout page itself cannot be styled from here** — it is Flitt's own page on `pay.flitt.com`, served in FONDY green, and the portal's `პერსონალიზაცია` tab is the only place that changes.
...
**Verified.** Against the real merchant: a checkout token mints; `fetchOrderStatus` answers `approved`/`GEL` for an order that exists and "Order not found" for one that does not — the same answer Flitt's public sandbox gives, which is what rules out our signature rather than assuming it. Through the real HTTP kernel, outside the test harness so CSRF genuinely runs: `POST /checkout/complete` is 200 with the app shell where it was 405, `GET` unchanged at 200, `/marketplace` unchanged, and `POST /marketplace` still 405 — the exemption did not spread. `php artisan test` 155/155, eight of them new: three pinning the 405 and five the language — that an unknown value falls back instead of failing the payment, that an array where a string was expected does the same, and that `lang` sits inside the signed set rather than beside it. `tsc --noEmit` and `eslint` clean on both changed pages. Observed on Flitt's page for 4057819: card fields and a Google Pay button, Google Pay itself reporting "won't be charged because you're in a test environment"; Apple Pay completed from an iPhone; a card payment completed and recorded as `payment_system: card`, `payment_id 1017638341`.

**Not verified:** that the checkout page actually comes back in Georgian. The deployed bundle was read back from kereforyou.com and does send `body: {lang}` from both call sites, and `lang` is in the gateway's documented order parameters with the signature covering it — but the one checkout observed after the deploy was still rendered in English, on a session whose language could not be established after the fact. Minting a token to watch it was not something this session could do. One payment with the site set to `ქართ` settles it.

**All three methods are confirmed on merchant 4057819**, each carrying its own mark in Flitt's payment list: card `1017645723` (ORD-JDEUP70C, ₾138, Visa), Google Pay `1017642722` (ORD-HREYDSJ3, ₾315) and Apple Pay `1017632236` (₾1). Every one `approved`. Flitt's API reports `payment_system: card` for all of them — the wallet shows in the portal's icon and the masked card (`543035…7000` for Apple Pay against `411111…1111` for the rest), not in that field, which is worth knowing before reading anything into it.
...
- **Fixed after deploy: live studio orders showed only placeholders.** `storeCustomOrder` validates each key of `customization` separately, and `color_name` had no rule, so `validate()` silently dropped it from every studio order. With no colour name, the "photographed in the chosen colour" test failed and every card fell back to the placeholder. My pre-deploy check had inserted orders directly, skipping validation, so it never saw this. Now the rule exists, so new orders keep the colour. Orders already placed recover it from the spec's colour line (`COLOUR_SPEC_ATTRIBUTE`, shared with `useCustomizer`'s spec builder so the two cannot drift). A regression test posts a studio order through the API and fails without the rule. Verified in Chrome: an order stored without `color_name` renders its photograph.
...
**The VISA/MC marks are now real artwork**, replacing the words `VISA` and `MC` in 10px text beside a generic Lucide card icon. Both were supplied as stock downloads and neither was usable as it arrived:

- **The Mastercard file had no transparency at all.** Its name says `transparent-background`, but every one of its 640,000 pixels is opaque and the corners alternate `#ffffff` / `#ebebeb` — the checkerboard is *painted into the image*. Dropped into the footer it would have rendered as a grey chequered tile, on the page a card reviewer opens first. `sharp`'s `trim()` finding nothing to cut across the full 800×800 is what gave it away. Recovered by flood-filling from the border through connected background only: knocking out every white and near-white pixel would also have eaten the white "MasterCard" lettering, but the circles enclose it, so a border-seeded fill stops at the artwork. 70% of the canvas came away and the lettering survived.
- **Both were the wrong size for their job.** Visa arrived as a 3840×2160 canvas — 29% artwork, the rest empty — which at footer size would have rendered a tiny mark adrift in its own padding. Trimmed on alpha and resized to 64px tall (2× the rendered height, for retina), they are 198×64 at 8.7 KB and 106×64 at 9.2 KB, down from 30 KB and 212 KB.

Served from `public/assets/payment/`, not the `garments/Logos/` folder they were dropped into — that directory is garment photography, and these are neither.

**Both marks were replaced with current artwork the same day** (2026-09-25), which is what removed the one real objection a reviewer could have made. Mastercard is now the post-2016 symbol — two flat interlocking circles, no lettering — rather than the retired "MasterCard" mark with the stripes, and Visa is the 2021 wordmark rather than the 2006 blue-and-gold flag. Both arrived as genuine RGBA this time, so they needed only the same trim-and-resize as before: 198×64 at 7.2 KB and 102×64 at 2.9 KB. The footer's intrinsic `width` moved 106 → 102 with the Mastercard's new aspect ratio, so the reserved box still matches the image and nothing shifts as it loads.
...
- **Imported wholesale (design only):** `app.css` (the bulk of the redesign — a warm `--store-*` storefront palette), `ui/button.tsx` (`bg-primary` → `bg-brand`, plus `data-kere-button`/`data-variant`/`data-size` hooks), `ui/dialog.tsx`, both locale files (+48 keys, en/ka verified key-for-key at 1551 each), the tailor dashboard components, and ~40 pages. New components `BrandStorySection`, `ProductImage`. `eslint.config.js` gains `scripts/**/*.mjs` to the node globals, which `main`'s `import-studio-drop.mjs` needed.
- **Functions kept from main (her branch removed them):** the `NewsletterPopup` — she had stubbed the whole component to `return null` and dropped it plus `JoinSection` from `Landing`; both are restored and rendered. The designer's footer **Back** button. The stage caption system (`caption` state, `stageEyebrow`/`stageTitle`, and `StagePanel`'s `eyebrow`/`title` props), re-sited into her new sticky preview panel — desktop only, because her phone stage is a 34vh strip with no room for it. The adaptive light/dark navbar (`isDarkElement` luminance probe, `updateTone`, `data-nav-theme`), grafted onto her new `.store-header` with a matching `[data-nav-tone="dark"]` rule, since her header was single-tone.
- **Reverted revert:** her `attributeGroups` predates `1f773ea` and would have dropped `.filter(attribute => attribute.options.length > 0)`, putting empty attribute headings back. Main's version is preserved.
- **Functions accepted (additive):** the **wishlist** — `wishlist_items` table (composite unique, cascade delete), `WishlistController` scoped to `$request->user()`, three routes behind `auth.bearer`, `/wishlist` page. The consent banner's **preferences** panel. The designer's `changingCategory` progressive disclosure and scroll-into-view. `ProductController`'s `customizable` filter and category search, with `StorefrontSearchTest` covering both.
- **Bugs fixed in her work:** `User::wishlistProducts()` was never written, so all three wishlist endpoints would have thrown `BadMethodCallException` — every one of them a 500. Added the `belongsToMany`. The consent banner's new close button called `setVisible(false)` and recorded nothing, which is not a decision and would have re-prompted on every load; it now declines. The cart drawer's checkout button lost `w-full` and would have shrunk to its text. Her duplicate `skeletons/ProductCardSkeleton` (generic slate, `rounded-xl`, 3:4) replaced the purpose-built warm `--kd-*` 4:5 one on the marketplace grid — the duplicate is deleted and the original restored. A raw `<button>` for "change category" is now `<Button variant="link">`, and the product-grid stagger is back to the spec's `0.5`.

**Verified:** `tsc --noEmit` clean; `eslint` 0 errors (7 warnings, all pre-existing in untouched files); `vite build` clean at 2198 modules; `php artisan test` 84/84. Wishlist exercised over HTTP end to end — 401 unauthenticated, 201 on add, idempotent on repeat, 200 with payload on read, 200 on delete, empty after. Relation and the `withAvg`/pivot-ordered index query confirmed in tinker. **Not verified:** no browser pass — there is no browser automation in this project, so the visual result of the redesign is unconfirmed.

**Known follow-up:** `Navigation.tsx` now emits `.store-header`, so the `.kere-site-header` blocks in `app.css` (~100 lines around 602–698 and 1328–1329) are dead. Left in place rather than deleted without a browser to confirm nothing still matches them. Her `AnimatedNavText` removal was not reverted — her `.store-desktop-nav` carries its own hover treatment and the old font-size growth would fight it. Several raw styled `<button>`s remain in her CSS-class-driven markup (`market-filter-chip`, `cookie-close`, `store-icon`), which is a §25 exception her approach introduces wholesale.

**QA round — two fixes.**

- **The selected tile collapsed, because brand colour was being read as "this is a CTA".** A rule in `app.css` sized actions to their labels — `width: fit-content !important; flex: 0 1 auto !important` — off a selector list that included `:is(button, a)[class~="bg-brand"]`. `OptionTile` wears `bg-brand` when selected, as a fill and nothing more, so the selected tile shrank to its text while its siblings stayed at full column width: Cap 52px against 395px. That is precisely what `1f773ea` had set out to stop. The clause is removed; `[data-kere-button]` was already in the same list, so every real `<Button>` keeps its sizing and the fill goes back to meaning fill. The blast radius was wider than the designer: every selected tab, filter chip and toggle in the app wears `bg-brand` too — `DesignCanvas`, `AddProductModal`, `OrdersList`, `AdminDashboard`, `CustomizerAdminPage`, `ProductCustomization`'s size row — and each was collapsing on selection. Nothing depended on the clause: every `<a>`/`<Link>` using it is already `inline-block` or `inline-flex`, and no raw element pairs it with `w-full`. The `bg-[#631e26]`/`bg-[#6F1D24]` clauses are hex-specific, sit on genuine CTAs, and are left alone.
- **The public catalogue was publishing tailors' personal data.** `/api/products` needs no authentication and eager-loaded the whole `tailor` relation, so an unauthenticated request returned `email`, `phone`, `workspace_address`, `legal_status`, `business_type`, `date_of_birth`, `approval_status`, `is_suspended` — and, for a tailor who is a minor, `guardian_name`, `guardian_email`, `guardian_phone` and `guardian_relationship`, which are a third party's contact details. The relation hole predates this work, but `f2f1d7e` moved address, phone, legal status and DOB into `$fillable` and so into the payload, and `WishlistController` inherited the pattern. Fixed with `User::PUBLIC_TAILOR_COLUMNS`, an allowlist applied to all four catalogue load sites (`ProductController::index`, `::show`, its related query, and `WishlistController::index`). Selecting rather than hiding is deliberate: `$hidden` cannot be used, since these are fields the owner's own screens legitimately read, and a column added to `users` later is now private by default instead of public by accident. The admin and customer-order paths are untouched — `AdminDashboard` is the only screen reading `tailor.email`/`phone`, and it goes through `AdminController`.
- Marketplace's two `duration: 0.15` popover transitions moved to `0.2`, matching the modal/disclosure micro-interactions already in the codebase.

**Verified:** `CatalogueTailorPrivacyTest` added — four cases covering the list, the single product and its related products, an authenticated wishlist read, and that the storefront still receives `bio`/`specialty`/`years_experience`/`tailor_name`. Confirmed as a real guard by reverting the allowlist and watching the two catalogue cases fail, then restoring. Suite now 88 passing. Leak closed and re-checked over HTTP on all four endpoints: the tailor object is down to the ten allowlisted columns with no sensitive field present. `tsc`, `eslint` (0 errors), `vite build` all clean.

**Still open from QA, not addressed here:** object-storage's `private/` prefix is enforced by bucket and domain configuration rather than by code — on R2 a per-object `'visibility' => 'private'` is a no-op, both disks point at one `R2_BUCKET`, and the seven `UploadStorageTest` cases run against `Storage::fake` on the local disks, so they cover the fallback rather than the object-storage behaviour. Nothing in `app/` or the frontend ever reads `id_document_path`, so today verification means opening the bucket by hand. Separate buckets, or a signed-URL-only custom domain, would make the property hold in code; that is an infrastructure decision, not a patch. Product image URLs are still persisted absolute, so changing `APP_URL`/`R2_PUBLIC_URL` breaks stored rows. A selected tile also picks up the CTA hover colour on hover (`--store-button-hover`) from a second, colour-keyed rule — cosmetic, same root pattern, left for a browser pass.
...
### 2026-08-22 — Price row moves into the flow; CTA renamed to Continue

- **The mobile price row is no longer fixed to the viewport.** It now sits at the end of the options column, so it no longer floats over the page. It had to move *inside* that column: the customizer's root is a two-column grid, and as a grid sibling an unfixed row would have claimed a cell of its own on desktop.
- With nothing fixed at the bottom any more, the `--kere-bottom-bar` machinery added earlier the same day is gone — the Customizer no longer publishes the variable and the consent banner is back to a plain `bottom-0`. The banner's tightened mobile padding stays. Clearance now comes from the column's own bottom padding, so the banner cannot sit on the price row at the end of the page.
- **The primary button no longer says "Order".** It reads *Continue* (`გაგრძელება`), and *Continue — ₾45.00* on desktop. "Add to cart" was considered and rejected: the cart is explicitly for marketplace products — `CartItem` carries a `productId`, size, colour and quantity, with no shape for a custom design's configuration — and the button goes to tailor selection, not a cart. Labelling it "Add to cart" would have described something the button does not do. The old `customizer.order` / `orderPrice` keys were removed from both locales.
- **A 2px overflow this introduced, caught and fixed before shipping**: every child of the price row is `shrink-0`, so the Georgian labels (`სულ ₾45.00 გაგრძელება` plus two icon buttons) pushed it to 362px on a 360px screen. Tighter gap on phones fixes it; `flex-wrap` keeps it safe for longer prices or labels.
- **Verified**: nothing fixed to the bottom at 360/390/1440 in either locale, all `position: static`; 0px horizontal overflow everywhere (measured against `clientWidth`); CTA reads Continue on phone and Continue — ₾45.00 on desktop, translated, with no leftover `შეკვეთა` and no raw i18n keys; no console errors. All ten earlier suites re-run green.

### 2026-08-22 — Code review fixes: the studio order now reaches the tailor

Three blockers from the review of the women's-designer work. All three were on the exit seam — the studio captured a rich configuration and the order flow dropped it.

- **The configuration never reached the tailor.** `getConfiguration()` returned raw ids (`{categoryId: optionId}`), and nothing rendered them: `OrdersList` showed fabric/colour/measurements but not the customization, `AvailableDesigns` — the bidding view — did not even declare the field, and `OrderReview` used it only as a truthy flag to print "Custom design from studio". A tailor priced a Corset Top seeing "Custom: shirt" and four measurements. Ids are also not a durable record: retiring an option leaves historical orders pointing at an unresolvable row.
  - `DesignConfiguration` now carries a `spec` snapshot — `{ attribute, option, price_modifier }` per line, taken at order time from the labels already on the client, keeping the ids alongside for reopening the design. New `DesignSpecList` renders it in all three places. Sub-selected children win over their parent and the chosen colour folds into the style line.
  - Bounded server-side (`max:30` lines, typed and length-capped) — the parent array only limits its top-level key count, so nested content was otherwise unvalidated free-form JSON. Payloads without a spec still validate, so existing orders are unaffected.
- **Three different prices.** `estimated_price` was `product.base_price`, discarding the configured total the customer had just read. Verified on `womens-blouse`: CTA showed ₾151, review quoted ₾85 + ₾15 shipping = ₾100 — a 44% understatement. Harmless before this line of work, because every men's option has a modifier of 0; the new attributes add up to +₾66. `onOrder` now receives the live total and the draft records it.
- **`garment_type` was never set on the `/customize/:slug` path.** Only `ProductStep` set it, but the customizer is also reached from "Edit design" in My Designs and by direct link. Fresh tab: `TailorSelectStep` saw an empty value and bounced to `/design`, losing every choice. Same session, worse: a draft left over from browsing Blazers filed a camisole order as a jacket and matched it against jacket tailors. `CustomizePage` now derives it from the taxonomy it already looks up for the breadcrumb, falling back to the product's own category.
- **Verified end to end**: configured a blouse to ₾151, confirmed the draft carries `garment_type: shirt`, `estimated_price: 151`, and a five-line spec with readable labels and modifiers (`Neckline: Sweetheart (+₾12)`, `Back Design: Lace-up (+₾20)`) alongside the raw ids; OrderReview renders the spec and quotes ₾151 + ₾15 = ₾166. Validation accepts a well-formed spec, rejects 31 lines and an overlong attribute, and still passes a legacy payload. Both tailor endpoints return `custom_design_data` wholesale, so the spec reaches `OrdersList` and `AvailableDesigns`.

Remaining review findings not addressed here — the client-side catalogue filter, the duplicated garment-label maps, the unenforced taxonomy/DB agreement, the dead `/api/customizer/preview` endpoint, `getConfiguration()` dropping `subSelections`, and the `'collar'` string literals — are logged for a cleanup pass. The label-map duplication should land before the second category seeder.

### 2026-08-24 — QA fixes: saved designs reopen intact, every advertised price is reachable

Five findings from the QA pass on the women's designer. Two were on the save/reopen seam, which had never actually worked end to end.

- **"Edit design" opened a default garment (H-1) — two faults stacked.** `CustomizePage` ignored the design being edited entirely: it rendered `/customize/:slug` and let the customizer seed itself from defaults, so every choice in a saved design was silently discarded the moment it was reopened. It now fetches `/api/customizer/designs/{id}` when `?design=` is present, holds the customizer behind a `designLoading` gate — mounting first would seed from defaults, then re-seed, flashing the wrong configuration and overwriting the session copy — and `useCustomizer` takes a `savedConfiguration` that outranks the per-tab session state, because a customer who asked for this design should not lose to a half-finished one left in the tab. `MyDesignsPage` links with the id.
- **The API could not have supplied the answer anyway (M-3).** `SavedDesignResource` returned `selections` as a **list**, not a map: `{110: 173}` came back as `[173]`, with the category ids — the half that says which attribute an option belongs to — gone. Root cause is in Laravel, not our code: `JsonResource::removeMissingValues()` runs `array_values()` when every key is numeric, and every key here is a database id. `preserveMaps()` now recurses the configuration and casts each non-list array back to an object. Confirmed on the wire for `selections`, `color_selections` and `sub_selections`.
- **The consent banner covered the Continue button on Georgian phones (M-4).** The earlier fix pointed the wrong way — the customizer reserved a fixed `pb-28` (112px) guess for a banner whose height it could not know. The banner now measures itself and publishes `--kere-consent-h`; `body` reserves exactly that. It matters because the guess was never close: the banner is 145px in English at 360px wide and **204px in Georgian**, where the copy wraps differently. The variable is cleared when the banner is dismissed or unmounts, so no other page inherits dead space.
- **Two garments advertised a price you could not reach (M-5).** Narrowing retires options per garment — an Off-shoulder Top has no plain neckline, a Tunic no short length — and those happened to be the zero-cost ones, so the cheapest configuration a customer could actually build sat above the price on the card. The seeder now subtracts the cheapest surviving modifier for each attribute, so a zero point always exists whatever narrowing removed. All 17 Tops now reach their advertised base exactly.
- **The design endpoint accepted any integer (L-6).** `configuration.*` was unvalidated free-form JSON: ids that never existed, ids belonging to a different garment, and unbounded maps all stored happily. Now bounded (`max:30`), checked for existence, and checked for **ownership** — every option must belong to the product being saved. Extended during verification, which found the first version incomplete: `sub_selections` is keyed by its parent option, so both sides of it are checked too, and a colour must belong to the very option it is keyed under, not merely to the same product.
- **Verified end to end.** Restore: in a tab with sessionStorage cleared, a saved design reopens with all six attributes and its colour intact (Oversized / Longline / Turtle / Cut-out / Bishop / Brown), demonstrably different from the defaults, and an unknown or foreign design id falls back to a usable garment rather than an error. Round trip through the UI itself: pick a non-default sleeve → Save Design → the row exists via the API → reopening restores that sleeve. Prices: all 17 Tops reach their base exactly, ₾45 T-shirt through ₾120 Evening Top. Banner: with Clarity configured against built assets — it cannot mount locally otherwise — at 360×740, 390×844 and 1440×900 in both locales, the reservation equals the measured height and, scrolled to the end of the page, the CTA hit-tests to itself with its bottom edge above the banner's top edge. Validation: foreign option, foreign sub-selection, mismatched colour pair, unknown ids and 31 selections all rejected; legitimate saves still pass. All twelve earlier browser suites re-run green.
- **Test-harness notes.** Three false results, all from the probe rather than the product: selecting the CTA by document order picked the `lg:hidden` mobile row, which is `display: none` on desktop and answers every geometric question with zeros — pick by measured size instead; `scrollIntoView({ block: 'end' })` parks the button flush against the viewport bottom, i.e. under the banner by construction, so the assertion must scroll to the end of the document the way a customer does; and `\s` inside a plain template literal is eaten before the page sees it, which silently turns a regex into a different regex. `String.raw` for anything evaluated in the browser.
- **Pre-existing, noted not fixed** (outside this change, flagged for a cleanup pass): `FinalPreview.tsx:248` passes `title` to a Lucide icon and `MeasurementGuideModal.tsx:113` references the `JSX` namespace — both fail a strict `tsc --noEmit`, though Vite builds fine since esbuild does not typecheck. The landing page also logs a Motion "container has a non-static position" warning and one testimonial avatar from `images.unsplash.com` is blocked by ORB; both reach production consoles.

### 2026-08-24 — Reverted: the second T-shirt shoot and everything built on it

Reverted at the customer's request, back to the state before the second batch of T-shirt photographs arrived. Five commits undone: the re-shoot, the sleeve-fit model, the Style restoration, and the move of sleeve fit inside Sleeves.

- **The T-shirt is as it was**: the `Classic` style layer carrying the original nine colours from `WomanTshirtClassic/`, five attributes plus Style, no sub-options, `Cap` as the photographed sleeve, ₾45.
