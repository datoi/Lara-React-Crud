...
**Worth replacing before the review, even so:** the Mastercard artwork is the **pre-2016 mark** — "MasterCard" in camel case inside the interlocking circles, with the horizontal stripes. Mastercard retired it in 2016 for the current one: flat circles, lowercase "mastercard" set below them. Visa's is the 2006 blue-and-gold flag, superseded by the 2021 wordmark but still very widely used and far less likely to be questioned. Someone who works with card schemes daily will recognise the Mastercard one on sight. Swapping it is a one-file drop into `public/assets/payment/mastercard.png`; no markup changes.
