...
- **Harness note.** Five suites asserted a `Style` screen that no longer exists, and one pinned the old `Cap`. They were updated rather than worked around — but the deeper lesson is the first bullet: a measurement can be precise, reproducible and aimed at the wrong property, and it reads exactly like evidence.
