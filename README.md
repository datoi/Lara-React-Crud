...
### 2026-08-24 — Second T-shirt shoot: same cut, 23 colours, verified against its own filenames

Asked to check `WomanTshirtClassic` and `WomanTshirtCassic2` and file each into the options its filenames describe.

- **The filenames encode the spec**, in our own attribute order: `Style_Fit_Length_Neckline_Sleeves_Back_Colour_View`. The first batch reads `Classic_BodyFit_Above_Crew_Cap_Classic`, the second `Classic_Fit_Above_Crew_Cap_Fitted` — the same garment described twice, with "BodyFit" and "Fitted" both meaning body-fitting and "Above" meaning above the waist.
- **They are the same cut, measured, not eyeballed.** Rendering both at matched scale suggested the second had longer sleeves; sampling the pixels said otherwise. Garment bounding box, where the sleeves stop as a fraction of body height, and hem-to-shoulder ratio, across three shared colours: sleeves end at 46.2–46.6% in the first batch and 48.2–48.5% in the second, hem/shoulder 0.50 vs 0.53, aspect 1.33 vs 1.27. A different length or sleeve option would differ by tens of percent, not two. The second shoot is the same garment photographed better — 1254px instead of 700px, cleaner lighting — in 23 colours instead of 9.
- **So there is no second option to file it under.** It replaces the first batch rather than sitting beside it: two near-identical Classic styles in one picker would be a choice with no difference. Same folder name, so every stored image path stays valid.
- **The existing mapping was already correct** — Classic / Body-Fitting / Cropped / Crew / Cap — with one gap: `Back Design` was never pinned, though both batches name a classic back. Now pinned to *Normal / No change*, so all six rows of the details panel state what the photograph actually shows instead of five.
- **Colours: 9 → 23**, ordered darkest neutrals through brights with Burgundy still first, so the card preview and the opening view are unchanged. Every swatch hex is sampled from its own photograph, and each colour word was checked against the pixels it labels — a "Navy" that was really black would put a customer's choice on the wrong swatch.
- **A dropped colour is now deleted, not left behind.** The first batch had a Green the second does not; `updateOrCreate` would have kept that row pointing at a file the new set no longer contains, showing a broken swatch. `seedPhotoLayer` removes colours absent from the list.
- 122.8MB of masters compress to 6.60MB of web assets (92 files, 23 colours × 4 views). The masters folder is gitignored like the first batch. The three-quarter view still has no slot in the schema, so 23 source files go unused.
- **Verified**: 23 swatches render and **each one loads its own photograph** — swept all 23 and polled for the repaint, because the canvas deliberately holds the previous photo until the new one has loaded and a fixed wait reports false mismatches; all four camera angles resolve; no broken images anywhere. Phone layout survives a colour block that grew from two rows to five: at 360×740 and 390×844 the garment and its colours are still on screen together (the preview is pinned above them), with no horizontal overflow, and the same in Georgian. Hoodie, Bodysuit, the placeholder states, the men's chino and the ₾45 price are all unchanged. Six earlier suites re-run green.
