...
### 2026-08-24 — Two T-shirt shoots, and the sleeve fit that tells them apart

Asked to check `WomanTshirtClassic` and `WomanTshirtCassic2` and file each into the options its filenames describe.

- **I first concluded they were the same cut. They are not.** The filenames say so only faintly — `Classic_BodyFit_Above_Crew_Cap_Classic` against `Classic_Fit_Above_Crew_Cap_Fitted` — and my measurements compared sleeve *length*, hem-to-shoulder ratio and bounding box, which agree to within two percent. All true, and all beside the point: relaxed and fitted sleeves differ in **volume**, not length. Cropping the same sleeve out of both shoots at matched scale settles it in one look — the first stands off the arm with a wide, draped opening, the second tapers to follow it. The customer said as much; the pixels back it up once you measure the right property.
- **So the photography varies along an attribute, and now it is modelled that way.** A new **Sleeve Fit** attribute — *Fitted* / *Relaxed* — carries the two shoots as its options, each with its own photographs and its own colours. Choosing one swaps the garment on screen. This is what the previous conclusion would have thrown away: the relaxed shoot was overwritten and had to be restored from git.
- **The retired `Style` layer.** The photography used to hang off a `Style` category holding a single option, `Classic`, that existed only to own the pictures. With the shoots differing by a real tailoring choice, that indirection is gone: the preview layer *is* the Sleeve Fit attribute, the customizer composites it exactly as before, and the details panel lists one fewer meaningless row. Seeding now prunes categories a garment no longer offers, so the old layer disappears on deploy rather than lingering.
- **Sleeve Fit is a line-wide attribute, not a T-shirt special case** — a sleeve can be cut close or cut with room on any garment that has one. Garments with no sleeves skip it, derived from their own sleeve options rather than a second hand-kept list of sleeveless garments, which would drift. Camisole, Tank Top and Vest correctly omit it; the other fourteen offer it.
- **The depicted sleeve was wrong: `Cap` → `Short`.** Both shoots name "Cap" in their filenames, but the sleeve covers the upper arm to mid-bicep, which is a short sleeve; a cap sleeve sits on the shoulder. The customer described the first shoot as "short" too. The garment now opens on Short, matching both the photograph and the line default.
- **Colours: 23 fitted, 9 relaxed**, each sampled from its own photograph and each colour word checked against the pixels it labels — a "Navy" that was really black would put a customer's choice on the wrong swatch. Switching between the shoots keeps the price at ₾45 and falls back cleanly when a colour exists in one set and not the other (the relaxed Green has no fitted counterpart). A dropped colour is deleted rather than left pointing at a file the new set no longer contains.
- **Folders now say what they hold**: `WomanTshirtFitted` (23 colours) and `WomanTshirtRelaxed` (9). 122.8MB of masters compress to 6.6MB of web assets; the masters folder is gitignored like the first batch, and the three-quarter view still has no slot in the schema.
- **Verified**: the garment opens on the fitted shoot with 23 colours at ₾45 and Short sleeves; picking Relaxed swaps the photograph and drops to that shoot's 9 colours; a relaxed-only colour renders, and switching back to Fitted recovers with nothing broken; Neckline, Sleeves and Back Design still show "not photographed yet"; Tank Top omits Sleeve Fit entirely; the men's chino is untouched at ₾120. Phone: the colour block grew from two rows to five and the page from 1105px to 1261px at 360×740, which is acceptable only because the preview is pinned — measured at 360×740, 390×844 and 414×896, the garment stays **fully** on screen while scrolling to the last colour and to the last attribute, so a customer always sees the change they are making. Saved designs restore all six attributes including Sleeve Fit. Eleven earlier suites re-run green.
- **Correction, same day: the Style row was never mine to remove.** Retiring it was a tidy-up nobody asked for, and it took a row off the details list the customer expected to be there. `Style / Classic` is back at the top, reading exactly as before. It is now a labelled choice rather than a canvas layer — the photography hangs off Sleeve Fit, and compositing two full-garment photos would stack one on the other — and it keeps a thumbnail, so opening it shows the garment rather than a placeholder. The seeder no longer prunes categories at all, so a re-seed cannot remove one from any garment. Sleeve Fit stays as what it always should have been: an addition.
- **The sleeve options were never touched.** All ten are on the T-shirt — Sleeveless, Cap, Short, Elbow, Three-quarter, Long, Puff, Bell, Balloon, Bishop — verified against the live API before and after. `One sleeve` is excluded for this garment, as it was long before this work.

- **Harness notes.** One suite pinned the old `Cap`, and several asserted against the Style screen while it was briefly gone; all updated. A suite also began failing because it asserted before the attribute tiles had rendered — the colour payload grew from 9 swatches to 32, so waiting for React's first paint is no longer enough; it waits for the customizer's own content now. The deeper lesson is the first bullet: a measurement can be precise, reproducible and aimed at the wrong property, and it reads exactly like evidence.

### 2026-08-24 — Sleeve fit moves inside Sleeves, and Style is retired

- **Style is gone.** It held a single option, `Classic`, that existed only to own the photographs — the garment *is* the style, and there was never a second one to pick. The details list is five rows now: Fit, Length, Neckline, Back Design, Sleeves.
- **Sleeve fit is a property of a sleeve, so it lives under one.** Each sleeve carries its own *Fitted* / *Relaxed* pair through the two-level picker the schema already had: choose the sleeve, then how it sits, on one screen under a "Sleeve fit" heading. `Sleeveless` alone has none — there is nothing to fit. The two shoots are the fits of the photographed sleeve, so picking one swaps the garment on screen and the colours with it.
- **Sub-option slugs carry their parent** (`short-fitted`, not `fitted`), because `layer_options` is unique on `(category, slug)` and ten sleeves would otherwise collide on one `fitted`. Found by the seeder failing on the second sleeve, not by reading the schema.
- **Option retirement had to be scoped to top-level options.** It deactivates anything whose slug is not in the garment's list — which is every sub-option, since theirs are parent-scoped. Unscoped, a re-seed would have retired every sleeve fit on the line.
- **Two gaps this opened, both closed:**
  - *An empty frame instead of a picture.* A preview layer whose selected option has no artwork paints nothing, and the photographed dimension is now a sleeve — so choosing a sleeve we have never shot left a blank box. The preview now checks whether the current choices resolve to an actual photograph, and shows the "not photographed yet" placeholder when they do not.
  - *The colour picker vanishing.* Colours hang off the photographed sub-options, so an unphotographed sleeve had none and the picker disappeared entirely — but colour is a property of the garment being ordered, not of the picture we happen to have. It now falls back to the garment's photographed option, so colour stays choosable whatever the sleeve.
- **The tile and the order spec name both halves.** A tile reading `Sleeves: Short` hid the fit that was chosen, and a tailor reading `Sleeves: Fitted` could not tell which sleeve was being fitted. Both now read `Short — Fitted`, with the sub-option's price folded in.
- **Migration, not seeder cleanup.** The retired `style` and `sleeve-fit` categories are removed by a one-off migration; the seeder only ever adds, so a database already seeded would have kept them forever. It is deliberately not reversible — those rows carried photography paths the model now writes elsewhere.
- **Verified**: five attributes, no Style, no separate Sleeve Fit; the Sleeves screen shows all ten sleeves plus both fits under a "Sleeve fit" heading; picking Relaxed swaps to the relaxed shoot and its 9 colours, Fitted back to 23; picking Bishop shows the placeholder rather than an empty frame, keeps the colour picker, and prices at ₾62; returning to Short restores the photograph; Neckline and Back Design still say "not photographed yet"; Tank Top's sleeveless option offers no fit; the men's chino is untouched at ₾120; 360px has no overflow. Twelve local suites re-run green.
- **Harness note.** Three suites carried the dead `main .lg\:sticky` selector — a fourth and fifth file after the two fixed this morning — which silently made every "coming soon" assertion false. One more compared a note against the *placeholder heading*, since both use the same words and the preview column comes first in the DOM; it now searches inside the options panel. Two suites also began failing intermittently on timing, because the colour payload grew again: they wait for the customizer's own content rather than React's first paint.
