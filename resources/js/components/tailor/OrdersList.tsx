...
function OrderDetailModal({ order, onClose, onStatusChange, currentUserId, initialTab = 'details' }: {
    ...
    const handleAction = async (newStatus: string) => {
        if (!onStatusChange || actioning) return;
            ...
            >
                ...
                <div className={activeTab === 'details' ? 'block' : 'hidden'}>
                    <div className="p-6 space-y-5">
                        ...
                        {/* Action buttons */}
                        {onStatusChange && !isFinal && (
                            <div className="space-y-3 pt-1">
                                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">
                                    {t('tailorComponents.orderActions')}
                                </p>
                                <div className="flex items-center gap-3 flex-wrap">
                                    {order.status === 'pending' && (
                                        ...
                                        >
                                            {actioning === 'processing'
                                                ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {t('tailorComponents.accepting')}</>
                                                : t('tailorComponents.acceptOrder')
                                            }
                                        </Button>
                                    )}
                                    ...
                                    {order.status === 'processing' && (
                                        ...
                                        >
                                            {actioning === 'finished'
                                                ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {t('tailorComponents.completing')}</>
                                                : t('tailorComponents.completeOrder')
                                            }
                                        </Button>
                                    )}
                                    ...
                                    {(order.status === 'pending' || order.status === 'processing') && (
                                        ...
                                        >
                                            {actioning === 'cancelled'
                                                ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {t('tailorComponents.cancelling')}</>
                                                : t('tailorComponents.cancelOrder')
                                            }
                                        </Button>
                                    )}
